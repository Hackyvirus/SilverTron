...
import {
  Bot, ChartNoAxesCombined, MessageCircle, User, LogOut, Menu, X,
  Bell, CreditCard, BarChart
} from 'lucide-react'

import MyAccountPage from '@/components/AccountPage'
import ProfilePage from '@/components/ProfilePage'
import AnalyticsPage from '@/components/UserAnalyticsPage'
...
export default function DashboardLayout() {
  ...
  const renderContent = () => {
    if (!profileSubmitted && activeTab === 'About') {
      return <ProfilePage key="about" onSubmit={handleProfileSubmit} />
    }

    switch (activeTab) {
      case 'Analytics':
        return <AnalyticsPage key="analytics" />
      case 'WithdrawalRequestForm':
        return <WithdrawalRequestForm key="WithdrawalRequestForm" onSubmit={() => setActiveTab('Analytics')} />
      case 'Chatbot':
        return <ChatbotPage key="chatbot" />
      case 'Chats':
        return <ChatsPage key="chats" />
      case 'Account':
        return <MyAccountPage key="account" />
      case 'Notifications':
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 p-4 space-y-2 overflow-y-auto">
        ...
        <SidebarItem 
          ...
          onClick={() => { setActiveTab('WithdrawalRequestForm'); setSidebarOpen(false); }} 
          isActive={activeTab === 'WithdrawalRequestForm'} 
        />
